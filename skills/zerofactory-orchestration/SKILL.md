...
2. **Actionable Backlog**: Actionable child tasks and autonomous codebase improvement tasks filed by `zf-orchestrator` enter `Todo`.
3. **Autonomous Execution**: The kanban dispatcher automatically provisions isolated Git worktrees, pulls/merges the latest default branch, and spawns `zf-builder`, moving tasks to `Running`.
4. **PR Creation & Continuous Review**: When `zf-builder` completes the work, the dispatcher commits the branch, opens a GitHub Pull Request, and routes the ticket to `zf-reviewer` in `Running`. `zf-reviewer` runs multi-round review (up to 3 rounds) requesting changes or approving.
