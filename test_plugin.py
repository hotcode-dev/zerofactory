...
class TestZeroFactory(unittest.TestCase):
    ...
    def test_45b_legacy_ready_status_migration(self):
        """Regression: DB rows stuck in the legacy 'ready' status (removed in
        the ready-column refactor) must be migrated to 'todo', audited via a
        task_activity row, and then dispatched — instead of being silently
        orphaned (the dispatcher only promotes 'todo' rows and the CLI/API
        reject 'ready', so without a migration such rows are invisible to
        dispatch forever).

        Covers both migration sources: init_db() (API/dashboard path) and the
        defensive re-mapping inside run_dispatch_cycle() (dispatcher-only path).
        """
        import shutil
        import sqlite3
        import tempfile
        ...
        os.environ["ZEROFACTORY_SKIP_WORKER_SPAWN"] = "1"
        old_db = os.environ.get("ZEROFACTORY_DB")
        td = tempfile.mkdtemp(prefix="zf-legacy-ready-migrate-")
        db_file = Path(td) / "legacy_ready.db"
        try:
            # --- Seed a pre-refactor DB: one legacy 'ready' row ---
            ws = Path(td) / "ws"
            ws.mkdir()
            ...
            conn.execute("CREATE TABLE task_links (id INTEGER PRIMARY KEY, parent_id TEXT, child_id TEXT, created_at INTEGER NOT NULL)")
            conn.execute("CREATE TABLE settings (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at INTEGER NOT NULL)")
            conn.execute("INSERT INTO boards (slug, created_at, updated_at) VALUES ('b1', 1, 1)")
            conn.execute("INSERT INTO tasks (id, board_slug, title, status, assignee, priority, workspace_path, created_at, updated_at) VALUES ('legacy-1', 'b1', 'Stuck ready task', 'ready', 'zf-builder', 'P2', ?, 1000, 1000)", (str(ws),))
            conn.commit()
            conn.close()

            # --- Migration source 1: init_db() normalizes the row in place ---
            ...
            conn = sqlite3.connect(str(db_file))
            conn.row_factory = sqlite3.Row
            status = conn.execute("SELECT status FROM tasks WHERE id = 'legacy-1'").fetchone()["status"]
            acts = conn.execute(
                "SELECT actor, action, details FROM task_activity WHERE task_id = 'legacy-1' AND action = 'migrate'"
            ).fetchall()
            conn.close()
            self.assertEqual(status, "todo", "init_db() must migrate legacy 'ready' rows to 'todo'")
            self.assertEqual(len(acts), 1, "migrated row must leave an auditable task_activity entry")
            self.assertIn("ready", acts[0]["details"])
            self.assertIn("todo", acts[0]["details"])

            # --- Migration source 2: run_dispatch_cycle() dispatches the row ---
            with patch("dispatcher.reap_active_workers", return_value=0), \
                 patch("dispatcher.spawn_agent_worker", return_value=(99801, "sess-legacy")):
                res = run_dispatch_cycle(db_file)
            self.assertTrue(res["ok"], f"dispatch cycle should succeed: {res}")
            # init_db() already migrated the row, so the cycle's own
            # re-mapping is a no-op this time.
            self.assertEqual(res.get("migrated_ready", 0), 0)
            conn = sqlite3.connect(str(db_file))
            conn.row_factory = sqlite3.Row
            row = conn.execute("SELECT status, metadata FROM tasks WHERE id = 'legacy-1'").fetchone()
            conn.close()
            self.assertEqual(row["status"], "running", "migrated legacy task must be dispatched to running")
            import json as _json
            self.assertEqual(_json.loads(row["metadata"]).get("worker_pid"), 99801)

            # --- Second cycle: idempotent, nothing left to migrate ---
            with patch("dispatcher.reap_active_workers", return_value=0), \
                 patch("dispatcher.spawn_agent_worker"):
                res2 = run_dispatch_cycle(db_file)
            self.assertTrue(res2["ok"])
            self.assertEqual(res2.get("migrated_ready", 0), 0)
        finally:
            shutil.rmtree(td, ignore_errors=True)
            ...
            # --- Migration source 2 (direct): a DB that init_db() never saw ---
            # Seed a fresh DB (full schema, as created pre-migration) with a
            # legacy 'ready' row and let ONLY run_dispatch_cycle() handle it:
            # the cycle must normalize the row AND dispatch it in one pass.
            td2 = tempfile.mkdtemp(prefix="zf-legacy-ready-dispatch-")
            db2 = Path(td2) / "legacy_ready2.db"
            try:
                ws2 = Path(td2) / "ws"
                ws2.mkdir()
                ...
                conn.execute("CREATE TABLE settings (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at INTEGER NOT NULL)")
                conn.execute("INSERT INTO boards (slug, created_at, updated_at) VALUES ('b1', 1, 1)")
                conn.execute("INSERT INTO tasks (id, board_slug, title, status, assignee, priority, workspace_path, created_at, updated_at) VALUES ('legacy-2', 'b1', 'Orphaned ready task', 'ready', 'zf-builder', 'P2', ?, 1000, 1000)", (str(ws2),))
                conn.commit()
                conn.close()

                with patch("dispatcher.reap_active_workers", return_value=0), \
                     patch("dispatcher.spawn_agent_worker", return_value=(99802, "sess-legacy2")):
                    res3 = run_dispatch_cycle(db2)
                self.assertTrue(res3["ok"], f"dispatch cycle should succeed: {res3}")
                self.assertEqual(res3.get("migrated_ready"), 1, "cycle must report the defensive migration")

                conn = sqlite3.connect(str(db2))
                conn.row_factory = sqlite3.Row
                row = conn.execute("SELECT status FROM tasks WHERE id = 'legacy-2'").fetchone()
                acts = conn.execute(
                    "SELECT actor, action FROM task_activity WHERE task_id = 'legacy-2' AND action = 'migrate'"
                ).fetchall()
                conn.close()
                self.assertEqual(row["status"], "running")
                self.assertEqual(len(acts), 1, "defensive migration must leave an auditable activity row")
            finally:
